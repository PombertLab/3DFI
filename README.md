...
```
###### Running trRosetta
Running [trRosetta](https://github.com/gjoni/trRosetta) involves 3 main steps: 1) searches with [HHsuite3](https://github.com/soedinglab/hh-suite)'s hhblits to generate .a3m alignment files; 2) the prediction protein inter-residue geometries with [trRosetta](https://github.com/gjoni/trRosetta)'s predict.py to generate .npz files, and 3) the prediction of 3D structures with trRosetta.py and [PyRosetta](http://www.pyrosetta.org/). Performing these predictions on several proteins can automated with 3DFI scripts.
...
```Bash
run_hhblits.pl -t 10 -f FASTA_OL/ -o HHBLITS/ -e 1e-40 1e-10 1e-03 -d /media/Data_3/Uniclust/UniRef30_2020_06
...
```Bash
create_npz.pl -a HHBLITS/*.a3m -o NPZ/ -p /media/Data_3/opt/trRosetta/network/predict.py -m /media/Data_3/opt/trRosetta/model2019_07
...
```Bash
create_pdb.pl -c 10 -n NPZ/*.npz -o PDB/ -f TEST/ -t /media/Data_3/opt/trRosetta/pdb/trRosetta.py
