...
## Modified to prevent problems with CPU (see https://github.com/RosettaCommons/trRosetta2/issues/3)
## Based on suggested fix by fbaltoumas on July 13, 2021 3:32AM CDT
## https://github.com/RosettaCommons/trRosetta2/issues/3#issuecomment-878889752
## 
## + changed CPU default to 10 (see below)

SCRIPT=`realpath -s $0`
export PIPEDIR=`dirname $SCRIPT`

CPU="10"  # number of CPUs to use
...
then
    ...
    python $PIPEDIR/trRefine/run_trRefine_DAN.py -msa_npz $WDIR/t000_.msa.npz \
		...
		--use_cpu -n_core $CPU > $WDIR/log/trRefine.stdout 2> $WDIR/log/trRefine.stderr
    cd -
fi
