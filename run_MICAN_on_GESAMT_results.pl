...
open IN, "<", "$tdfi/Homology/GESAMT/All_GESAMT_per_protein.tsv";
...
while(my $line = <IN>){
	...
	unless(($line =~ /^###/) || ($line eq "")){
		...
		system "mican -s $predicted_file_location $temp_file -n 1";
		...
		# my $result =`./mican -s $predicted_file $expiremental_file -n 1`;
		...
		# 		print("$rank\t$sTMscore\t$TMscore\t$Dali_Z\t$SPscore\t$Length\t$RMSD\t$Seq_Id\n");
		# 	}
		# }
	}
}
